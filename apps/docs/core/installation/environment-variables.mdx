...
Kaneo stores private uploads from task descriptions and task comments in either
S3-compatible object storage or, for single-instance deployments, a local
directory. See [Object storage and uploads](/core/installation/object-storage-and-image-uploads).

Name | Description | Default |
--- | --- | --- |
| `STORAGE_BACKEND` | Upload backend: `s3` or `local`. Unset keeps the historical S3 behavior so upgrades do not switch storage silently. | `s3` |
| `STORAGE_MAX_UPLOAD_BYTES` | Maximum allowed upload size in bytes for every backend. Falls back to `S3_MAX_IMAGE_UPLOAD_BYTES`. | `10485760` |
| `STORAGE_UPLOAD_URL_TTL_SECONDS` | How long an upload credential stays valid. Falls back to `S3_PRESIGN_TTL_SECONDS`. | `300` |
| `LOCAL_STORAGE_PATH` | Required when `STORAGE_BACKEND=local`. Absolute, writable directory for uploads. Must be a persistent volume; never a temporary directory. |  |
| `LOCAL_UPLOAD_TEMP_TTL_SECONDS` | Grace period before abandoned partial local uploads are cleaned up. | `3600` |
...
| `S3_MAX_IMAGE_UPLOAD_BYTES` | Legacy alias for `STORAGE_MAX_UPLOAD_BYTES`. | `10485760` |
...
| `S3_PRESIGN_TTL_SECONDS` | Legacy alias for `STORAGE_UPLOAD_URL_TTL_SECONDS`. | `300` |
