...
tools expect. Milestone dates (`startDate`, `targetDate`) accept calendar dates in
`YYYY-MM-DD` format (or UTC midnight `YYYY-MM-DDT00:00:00.000Z`). Non-zero time
or non-UTC offsets are rejected. Time entries have no delete endpoint on the API,
so there is no `delete_time_entry` tool.
